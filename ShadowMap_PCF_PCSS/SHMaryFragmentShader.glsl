...
float PCSS(vec3 lightDirection, vec3 normal)
{
	...
	float minFilterSize = 1.0f;
	minFilterSize = minFilterSize / float(SHADOW_MAP_SIZE);
	wPenumbra = max(minFilterSize, wPenumbra);
...
vec3 toSRGB(vec3 color)
{
	vec3 result;
	for (int i = 0; i < 3; i++)
		result[i] = pow(color[i], 1.0f / 2.2f);
	return result;
}
...
void main()
{
	...
	vec3 diffuse = vec3(texture(texture_diffuse1,  fsIn.texCoord));
